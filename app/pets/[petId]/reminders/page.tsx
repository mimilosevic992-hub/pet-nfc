...
function mapReminderTypeToSection(t: string): CompleteForm["section"] {
...
function badgeForStatus(s: string) {
  ...
  if (s === "overdue") return <span className={`${base} bg-red-100 text-red-800`}>OVERDUE</span>;
  if (s === "today") return <span className={`${base} bg-yellow-100 text-yellow-800`}>TODAY</span>;
...
export default function PetRemindersPage() {
  ...
  const [cf, setCf] = useState<CompleteForm>({
    section: "VACCINATION",
  ...
  async function completeReminder() {
    ...
    try {
      ...
      // backend traži da sekcija odgovara tipu:
      // VACCINE -> VACCINATION, CHECKUP -> CHECKUP, THERAPY -> THERAPY
      const expected = mapReminderTypeToSection(completeType);
      if (cf.section !== expected) {
      ...
      const res = await fetch(`${API_BASE}/reminders/${completeId}/complete_auth`, {
        ...
        body: JSON.stringify({
          ...
          title: cf.title,
          notes: cf.notes || null,
          vet_name: cf.vet_name || null,
          clinic: cf.clinic || null,
        }),
      });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    value={cf.section}
                    readOnly
                    title="Sekcija je zaključana i automatski zavisi od tipa podsetnika."
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    className="mt-1 w-full rounded-xl border px-3 py-2"
                    value={cf.vet_name}
                    onChange={(e) => setCf((p) => ({ ...p, vet_name: e.target.value }))}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    className="mt-1 w-full rounded-xl border px-3 py-2"
                    value={cf.clinic}
                    onChange={(e) => setCf((p) => ({ ...p, clinic: e.target.value }))}
                  />
                </div>
              </div>
              ...
              <button
                ...
                disabled={loading || !cf.title || !cf.date}
              ...
              <div className="mt-3 text-xs text-gray-500">
                Pravilo: kada završiš podsetnik → unosi se u zdravstveni karton i podsetnik se briše.
              </div>
            </div>
          </div>
        )}
